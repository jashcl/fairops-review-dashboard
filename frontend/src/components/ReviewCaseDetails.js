...
import { getRiskColor, getStatusColor } from '../utils/reviewCaseUtils';
...
const ReviewCaseDetails = () => {
  ...
  const handleStatusUpdate = async () => {
    ...
    if (requiresNote && !decisionNote.trim()) {
      setValidationError(`${nextStatus} decisions require a reviewer note.`);
      return;
    }

    if (nextStatus === 'Approved' && !allRequiredCompleted) {
      setValidationError('Approval is blocked until all required evidence items are completed.');
      return;
    }
  ...
  if (loading) {
    return <Box sx={{ display: 'flex', justifyContent: 'center', minHeight: 360, alignItems: 'center' }}><CircularProgress /></Box>;
  }
